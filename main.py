...
while True:
    ...
    clean_bytes = struct.pack(">iiiiiiii",
        int(light * 100), # Light intensity
        int(lux * 100), # lux amount of light
        int(roll * 100), # Roll in degrees in the range -180 to 180
        int(pitch * 100), # Pitch in degrees in the range -90 to 90
        int(acceleration_x * 100), # Acceleration x
        int(acceleration_y * 100), # Acceleration y
        int(acceleration_z * 100), # Acceleration z
        int(voltage * 100)) # Battery voltage
    s.send(clean_bytes)
    print("Bytes sent, sleeping for 5 secs")
